...
function SpellChecker(editor, root) {
	this.editor = editor;
	...
	for (var i in bl) {
		...
		if (!btn) {
			...
			var id = "SC-" + btn[0];
			cfg.registerButton(id, tt[id], root +"plugins/SpellChecker/img/" + btn[0] + ".gif", false,
					   function(editor, id) {
						   // dispatch button press event
	 				   self.buttonPress(editor, id, root);
					   }, btn[1]);
			toolbar.push(id);
		}
	}
...
SpellChecker.prototype.buttonPress = function(editor, id, root) {
	...
	switch (id) {
		...
		var uiurl = root + "plugins/SpellChecker/spell-check-ui.html";
		var win;
		if (HTMLArea.is_ie) {
