...
function update() {
  ...
  displayProgressBar(totalseconds-(remainingseconds/1000));
  ...
  if (remainingseconds > 0) {
    ...
    if ((warningMinutes * 60000) > remainingseconds) {
      ...
      if(ns4)
        PBdone.bgcolor = warningcolor;
      else
        PBdone.style.backgroundColor=warningcolor;
        newtag = opencolortags;
    }
    else {
      if(ns4)
        PBdone.bgcolor = loadedcolor;
      else
        PBdone.style.backgroundColor=loadedcolor
    }

    if (document.layers){
      document.countdownnsmain.document.countdownnssub.document.write(format_time_left())
      document.countdownnsmain.document.countdownnssub.document.close()
    }
    else if (document.all||document.getElementById) {
      crosscount.innerHTML=format_time_left()
    }
...
if(ns4){
  ...
  txt+='<ilayer name="PBouter" visibility="show" height="'+barheight+'" width="'+barwidth+'">';
  ...
  txt+='<table width="95%"><tr><td align="right"><div id="PBouter" style="position:relative; visibility:visible; background-color:'+bordercolor+'; width:'+barwidth+'px; height:'+barheight+'px;">';
