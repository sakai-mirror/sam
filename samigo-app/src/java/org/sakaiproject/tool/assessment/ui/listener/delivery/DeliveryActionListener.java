    ...
    try
    {
      ...
      case 5: // Take assessment via url
              ...
              if (log.isDebugEnabled()) {
            	  Integer setSize = 0; 
            	  if (ag.getItemGradingSet() != null) {
            		  setSize = ag.getItemGradingSet().size();
            	  }
            	  log.debug("**** DeliveryAction, itemgrading size="+ setSize);
              }
                      ...
                      HashMap itemGradingHash, boolean setNullOK)
  {
    ...
    if (keys.hasNext()) 
    {
      ItemGradingData igd = (ItemGradingData) ( (ArrayList) itemGradingHash.get(
        keys.next())).toArray()[0];
      AssessmentGradingData agd = gradingService.load(igd.getAssessmentGrading().getAssessmentGradingId().toString());
